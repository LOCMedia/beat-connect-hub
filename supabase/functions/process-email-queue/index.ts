...
function isForbidden(error: unknown): boolean {
  return error instanceof SendError && error.status === 403
}
function getRetryAfterSeconds(error: unknown): number {
  if (error instanceof SendError && error.retryAfterSeconds) return error.retryAfterSeconds
  return 60
}

const RESEND_URL = 'https://api.resend.com/emails'

async function sendViaResend(payload: Record<string, any>, resendKey: string, siteUrl: string) {
  const headers: Record<string, string> = {}
  if (payload.unsubscribe_token) {
    const unsubUrl = `${siteUrl}/unsubscribe-notifications?token=${payload.unsubscribe_token}`
  ...
  if (!res.ok) {
    ...
    throw new SendError(res.status, `Resend ${res.status}: ${body}`, retryAfter ? Number(retryAfter) : null)
  }
...
Deno.serve(async (req) => {
  ...
  const siteUrl = Deno.env.get('SITE_URL') || 'https://vibekonect.com'
  ...
  for (const queue of ['auth_emails', 'transactional_emails']) {
    ...
    for (let i = 0; i < messages.length; i++) {
      ...
      try {
        await sendViaResend(payload, resendKey, siteUrl)
